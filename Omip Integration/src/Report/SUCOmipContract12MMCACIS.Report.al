...
report 50165 "SUC Omip Contract 12M MC ACIS"
{
    Caption = 'Omip Contract Comercial';
    WordLayout = './src/Report/Layouts/Contracts/Acis/SUCOmipContract12MMCACIS.docx';
    DefaultLayout = Word;
    ...
    dataset
    {
        dataitem(SUCOmipEnergyContracts; "SUC Omip Energy Contracts")
        {
            ...
            column(CustomerManagerPosition; SUCOmipEnergyContracts."SUC Customer Manager Position") { }
            ...
            dataitem(SUCOmipPowerEntry; "SUC Omip Power Entry Contract")
            {
                ...
                column(DateCreatedPE; Format(SUCOmipEnergyContracts."Date Created", 0, '<Day> de <Month Text> de <Year4>')) { }
                column(CustomerManagerPE; CustomerManager) { }
                column(CustomerManagerVatRegNoPE; CustomerManagerPosition) { }
                column(CustomerManagerPositionPE; SUCOmipEnergyContracts."SUC Customer Manager Position") { }
                column(RefContract2; SUCOmipEnergyContracts."Reference Contract") { }
                column(TittleAppConditions; SUCOmipEnergyContracts."Tittle Applicable Conditions") { }
                column(ApplicableConditions; ApplicableConditions) { }
                dataitem(SUCOmipEnergyEntry; "SUC Omip Energy Entry Contract")
                {
                    DataItemTableView = sorting("Contract No.", "Rate No.", Times, Type);
                    DataItemLink = "Contract No." = field("Contract No."), "Rate No." = field("Rate No.");
                ...
                begin
                    ...
                    ApplicableConditions := SUCOmipEnergyContracts.GetDataValueBlob(SUCOmipEnergyContracts.FieldNo("Body Applicable Conditions"));
                end;
            }
            dataitem(SUCOmipEnergyContractsMul; "SUC Omip Energy Contracts Mul.")
            {
                DataItemTableView = sorting("Contract No.", "Customer CUPS");
                DataItemLink = "Contract No." = field("No.");
                column(CUPSM; "Customer CUPS") { }
                column(RateNo; "Rate No.") { }
                column(SPAddress; "SUC Supply Point Address") { }
                column(SPCity; "SUC Supply Point City") { }
                column(SPPostCode; "SUC Supply Point Post Code") { }
                column(P1; P1) { }
                column(P2; P2) { }
                column(P3; P3) { }
                column(P4; P4) { }
                column(P5; P5) { }
                column(P6; P6) { }
                column(ActDate; Format("Activation Date", 0, '<Day>/<Month>/<Year4>')) { }
            }
            trigger OnAfterGetRecord()
            begin
                Clear(CheckInv);
